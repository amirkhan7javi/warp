...
contract OracleEchidnaTest {
  ...
  function update(
    ...
    oracle.update(
      OracleTest.UpdateParams({advanceTimeBy: advanceTimeBy, tick: tick, liquidity: liquidity})
    );
  }
  ...
  function checkTimeWeightedResultAssertions(uint32 secondsAgo0, uint32 secondsAgo1) private view {
    ...
    (int56[] memory tickCumulatives, uint160[] memory secondsPerLiquidityCumulativeX128s) = oracle
      .observe(secondsAgos);
    ...
    uint256 timeWeightedHarmonicMeanLiquidity = (uint256(timeElapsed) * type(uint160).max) /
      (uint256(secondsPerLiquidityCumulativeX128s[1] - secondsPerLiquidityCumulativeX128s[0]) <<
        32);
  ...
  function checkTwoAdjacentObservationsTickCumulativeModTimeElapsedAlways0(uint16 index)
    external
    view
  {
    ...
    uint16 conditional = 0;
    if (index == 0) {
      conditional = cardinality - 1;
    } else {
      conditional = index - 1;
    }
    ...
    (uint32 blockTimestamp1, int56 tickCumulative1, , bool initialized1) = oracle.observations(
      index
    );
